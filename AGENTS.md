...
it with Codex Haiku, and writes it into a local markdown knowledge base (KB_DIR, default: `~/knowledge/`).
...
src/
  ...
  categorize.js    Codex Haiku (Codex-haiku-4-5-20251001). Returns category,
  ...
  Codex-print.js  Spawns `Codex -p` with prompt fed via stdin. Shared by
                   theme synthesis and project synthesis.
  synthesize.js    Two-tier theme wiki synthesis via `Codex -p`:
                   - Tier 1 (daily): parallel Codex -p per updated category →
                     updates wiki/{slug}.md
                   - Tier 2 (weekly): single Codex -p reads all wiki/ pages →
...
$KB_DIR/  (default: ~/knowledge/)
  ...
  projects/             # Per-project living docs (orthogonal to themes)
    AGENTS.md           # Layout and conventions for the projects subtree
    {name}/
...
Daily, after tier-1 theme synthesis, `synthesize-projects.js` runs `Codex -p` per project
...
- **`Codex -p` path** — `synthesize.js` gets CLAUDE_BIN from `src/config.js` (auto-detected via `which Codex`, or set CLAUDE_BIN in .env).
